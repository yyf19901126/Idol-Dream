...
function getWalk2(key){            // 只存 east(朝右)6帧；朝左用水平镜像(省一半生成)
  if(key in WALK2)return WALK2[key];
  if(!WALK2ING[key]){WALK2ING[key]=1;const e=new Array(6);let done=0;
    for(let i=0;i<6;i++){const im=new Image();
      im.onload=()=>{if(++done>=6)WALK2[key]=e.every(a=>a&&a.width)?{e}:null;};
      im.onerror=()=>{WALK2[key]=null;};
      im.src='art/chibi_walk2/'+key+'/e/f0'+i+'.png?v=1';e[i]=im;}}
  return WALK2[key];
}
function drawChibi(g,x,y,mods,o){
  ...
  if(w2&&w2.e){                                          // PixelLab 东西向：按朝向选 e/w，不翻转(本身就是左右朝向¾视角)
    const arr=w2.e,nf=arr.length,t=o.t||0,idx=o.moving?Math.floor(t*9)%nf:0,fim=arr[idx]||arr[0];
    ...
    g.save();g.globalAlpha=al;g.translate(Math.round(x),Math.round(y+10-hop-lift));if(o.face<0)g.scale(-1,1);g.imageSmoothingEnabled=false;   // 朝左→镜像east
